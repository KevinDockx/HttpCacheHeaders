...
public class HttpCacheHeadersMiddleware
{
    ...
    public async Task Invoke(HttpContext httpContext, IValidatorValueInvalidator validatorValueInvalidator)
    {
        ...
        InjectModelOptionsIfNecessary(httpContext);
    ...
    private void InjectModelOptionsIfNecessary(HttpContext httpContext)
    {
        // In case the resource filter is not executed in the filter pipeline, the correct expiration / validation
        // model options are missing from the httpContext.Items dictionary. In that case, get them from the endpoint.  
        // 
        var expiration = httpContext.GetEndpoint()?.Metadata.LastOrDefault(m => m is HttpCacheExpirationAttribute);
        var validation = httpContext.GetEndpoint()?.Metadata.LastOrDefault(m => m is HttpCacheValidationAttribute);

        // If we have the attribute but do not have the httpContext objects (in Minimal API), then add it
        if (expiration is not null && !httpContext.Items.ContainsKey(HttpContextExtensions.ContextItemsExpirationModelOptions))
        {
            httpContext.Items[HttpContextExtensions.ContextItemsExpirationModelOptions] =
              ((IModelOptionsProvider)expiration).GetModelOptions();
        }

        // If we have the attribute but do not have the httpContext objects (in Minimal API), then add it
        if (validation is not null && !httpContext.Items.ContainsKey(HttpContextExtensions.ContextItemsValidationModelOptions))
        {
            httpContext.Items[HttpContextExtensions.ContextItemsValidationModelOptions] =
              ((IModelOptionsProvider)validation).GetModelOptions();
        }
